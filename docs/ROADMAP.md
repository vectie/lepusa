...
- Implement:
  ...
  - `lepusa run`: prints a no-window native runner smoke summary with target launch readiness.
  ...
  - `lepusa launch-session [--async-bridge]`: emits a target-aware source launch readiness envelope with session JSON, launch capability, and optional async-capable bridge scheduler metadata.
  ...
  - `lepusa-runtime launch-session [--async-bridge]`: emits a packaged native-loop launch readiness envelope with session JSON, launch capability, and optional async-capable bridge scheduler metadata.
...
- `@lepusa/bundle` verifies materialized bundle files, resources, runtime
  manifests, nonblank initial WebView content, host-compatible runtime
  executable copies, and target launch capability after `bundle-write` as the
