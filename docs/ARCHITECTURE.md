...
- Built-in permission names for project manifests are `filesystem.read`,
  ...
  `process.info`, `process.environment`, and `process.control`; custom names use
  `custom:<name>`.
...
handoff routes. `@lepusa/plugins/dialog` declares message, confirm, and prompt
routes. `@lepusa/plugins/clipboard` and
...
official package names such as `deepLink`, `singleInstance`, `log`, `store`,
and `fs` to their official command contracts before planning, while plugins
with explicit `commands` arrays keep their manifest-defined surface.
