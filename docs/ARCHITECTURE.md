...
The macOS, Linux, and Windows C loops parse that operation, retain the
window/callback drain request in their native context, issue a `lepusa-drain-v1`
packet over the handoff callback, and execute the returned `lepusa-ops-v3`
operations from the platform event loop.
...
operations, bridge route scheduling, target-parameterized development command
templates, and a `RuntimeDevSession`. The dev session classifies each WebView's
frontend source, records whether reload is runner-owned or external-server
owned, exposes the reload event name, and links supervised localhost services
to their owning window. CLI, native runners, and external tooling consume that
object instead of reconstructing boot state or inferring frontend behavior from
raw URLs.
...
first `lepusa://` WebView through `launch_bundled(manifest)`.
...
window menu bars, and macOS/Linux apply status-item tray operations for icon,
tooltip, menu, menu popup, visibility, and destroy; macOS menu items also apply
supported accelerator strings as native key equivalents and modifier masks.
Rendered macOS app-menu/tray-menu items, Linux menu/tray action/check items,
and Windows menu action/check items dispatch `menu.onItemClick` or
`tray.onMenuItemClick` frontend events with the clicked item id. Remaining
Windows tray and non-macOS menu accelerator work is still reported as skipped
until concrete OS menu and status-item renderers land.
